...
class CmdAction(eg.ActionClass):
    ...
    def __call__(self):
        ...
        s.connect((self.plugin.host, self.plugin.port))
        s.send('ISCP\x00\x00\x00\x10\x00\x00\x00' + str(len(self.cmd)) + '\x01\x00\x00\x00' + self.cmd + '\r')
        s.close()
...
class ValueAction(eg.ActionWithStringParameter):
    ...
    def __call__(self, data):
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((self.plugin.host, self.plugin.port))
        s.send('ISCP\x00\x00\x00\x10\x00\x00\x00' + str(len(self.cmd) + len(data)) + '\x01\x00\x00\x00' + self.cmd + str(data) + '\r')
        s.close()
...
class Raw(eg.ActionWithStringParameter):
    ...
    def __call__(self, data):
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((self.plugin.host, self.plugin.port))
        s.send('ISCP\x00\x00\x00\x10\x00\x00\x00' + str(len(data)) + '\x01\x00\x00\x00!1' + str(data) + '\r')
        s.close()
