...
class TableCtrl(wx.ListCtrl):
    ...
    def FillData(self, data):
        ...
        for row in range(rows):
            self.InsertItem(row, data[row][0])
            for col in range(1, len(self.labels)):
                self.SetItem(row, col, data[row][col])
...
class VarTable(wx.ListCtrl, TextEditMixin):
    ...
    def __init__(self, parent, txt):
        ...
        self.SetToolTip(txt.varTableTip)
...
class Protocol(wx.Frame):
    ...
    def __init__(self, parent, plugin, title):
        ...
        idx0 = il.Add(wx.Bitmap(wx.Image(stream_op)))
        stream_var = StringIO(b64decode(VARIABLES))
        idx1 = il.Add(wx.Bitmap(wx.Image(stream_var)))
        ...
        if Persist.protServDim:
            self.SetSize(*Persist.protServDim)
        else:
            self.Center()
        self.MakeModal(True)
        self.Show(True)
    ...
    def CreateVarPanel(self):
        ...
        panel.varDel_Id = wx.NewIdRef()
        panel.varAdd_Id = wx.NewIdRef()
        panel.varClear_Id = wx.NewIdRef()
        ##panel.varRefr_Id = wx.NewIdRef()
        panel.varUp_Id = wx.NewIdRef()
        panel.varDown_Id = wx.NewIdRef()
        # Button UP
        bmp = wx.ArtProvider.GetBitmap(wx.ART_GO_UP, wx.ART_OTHER, (16, 16))
        ...
        panel.val_Id = wx.NewIdRef()
        self.Bind(wx.EVT_TEXT,self.onVarValue, id=panel.val_Id)
        self.Bind(wx.EVT_CHOICE, self.onVarValue, id=panel.val_Id)
    ...
    def CreateActionPanel(self):
        ...
        testBtn.SetToolTip(self.text.testToolTip)
    ...
    def OnGridRightClick(self, evt):  # Right click menu
        ...
        if not hasattr(self, "del_Id"):
            ...
            self.up_Id = wx.NewIdRef()
            self.down_Id = wx.NewIdRef()
            self.retRes_Id = wx.NewIdRef()
            self.autConn_Id = wx.NewIdRef()
    ...
    def onPlgnChoice(self, evt=None):
        ...
        choices.sort()
        actnsCtrl.AppendItems(items=choices)
        if evt:
    ...
    def __init__(self, plugin, title):
        ...
        def FillListCtrl(event=None):
            ...
            for i in range(cnt):
                ...
                ccListCtrl.InsertItem(i, "")  # Dummy column 0
                ccListCtrl.SetItem(i, 1, str(i + 1))
                ccListCtrl.SetItem(i, 2, ip)
                ccListCtrl.SetItem(i, 3, str(port))
                prot = str(self.plugin.clnProtocols[clnts[i]][0]) if self.plugin.clnProtocols[clnts[i]][
                    0] else text.oldProt
                ccListCtrl.SetItem(i, 4, prot)
            if event:
                event.Skip()
...
class WebsocketSuite(eg.PluginBase):
    ...
    def Configure(self, dataSet=[]):
        ...
        # setting cols width
        serverListCtrl.InsertItem(0, 30 * "X")
        serverListCtrl.SetItem(0, 1, " 255.255.255.255")
        serverListCtrl.SetItem(0, 2, "HHHHHHH")
        serverListCtrl.SetItem(0, 3, "X")
        serverListCtrl.SetItem(0, 4, 16 * "X")
        ...
        # setting cols width
        clientListCtrl.InsertItem(0, 30 * "X")
        clientListCtrl.SetItem(0, 1, " 255.255.255.255")
        clientListCtrl.SetItem(0, 2, "HHHHHHH")
        clientListCtrl.SetItem(0, 3, " 255.255.255.255")
        clientListCtrl.SetItem(0, 4, "HHHHHHH")
        clientListCtrl.SetItem(0, 5, 16 * "X")
        ...
        def PopulateListServ(event=None, toStop=()):
            ...
            for item in self.servers:
                if item[0] not in toStop:
                    ...
                    serverListCtrl.InsertItem(i, item[1])
                    serverListCtrl.SetItem(i, 1, item[0].getsockname()[0])
                    serverListCtrl.SetItem(i, 2, str(item[0].getsockname()[1]))
                    serverListCtrl.SetItem(i, 3, str(len(item[2])))
                    serverListCtrl.SetItem(i, 4, cls)
                    i += 1
        ...
        def PopulateListCli(event=None, toStop=()):
            ...
            for item in self.clients:
                if item[0] not in toStop:
                    clientListCtrl.InsertItem(i, item[1])
                    clientListCtrl.SetItem(i, 1, item[0].getsockname()[0])
                    clientListCtrl.SetItem(i, 2, str(item[0].getsockname()[1]))
                    clientListCtrl.SetItem(i, 3, item[0].getpeername()[0])
                    clientListCtrl.SetItem(i, 4, str(item[0].getpeername()[1]))
                    clientListCtrl.SetItem(i, 5, str(item[2]))
                    i += 1
                    ...
                    if r not in self.connected:  # first data => handshake
                        ...
                        if data and data != '\xff\x00':
                            ...
                            if self.InteractServer(data, r, ix):
                                log('closing after reading "I\'m leaving" [peer %s disconnected]' % str(
                                    r.getpeername()))
                                cl_close = True
                        # if no data or data == '\xff\x00':
                            ...
                            if data == '\xff\x00':
                                log('closing after "Close frame" from client [peer %s disconnected]' % str(
                                    r.getpeername()))
                            else:
                                log('closing after reading no data or invalid data [peer %s disconnected]' % str(
                                    r.getpeername()))
                            cl_close = True
                        if cl_close:
                            # Stop listening for input on the connection
