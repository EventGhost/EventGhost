...
from ctypes import byref, c_buffer, c_int, c_long, c_ulong, sizeof, Structure
...
from win32gui import GetMenuItemCount, GetParent
    ...
    def Configure(self, ArtsagePath=None, evtList=[[], [], [], [], [], []]):
        ...
        def OnPathChange(event=None):
            ...
            if event and not flag:
                wx.MessageBox(
                    message=self.text.boxMessage1 % (fPath, 'Artsage'),
                    caption=self.text.boxTitle,
                )
            if fPath != "":
