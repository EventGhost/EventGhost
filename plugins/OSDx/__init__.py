...
eg.RegisterPlugin(
    ...
    version = "0.01",
    kind = "other",
    guid = "{AAF7C455-DB27-497F-A8A3-BED65545AF3B}",
...
class OSDx(eg.PluginClass):
    def __init__(self):
        self.AddAction(ShowOSDx)
...
class ShowOSDx(eg.ActionBase):
    ...
    def Show(
        ...
        for i in reversed(xrange(len(self.frames))):
            if (not self.frames[i].timer.isAlive()):
                # print "DEL ", i
        ...
        for f in self.frames:
            if (f.displayNumber == displayNumber) and (f.alignment == alignment):
                ff = f
                break

        if (ff is None):
            # print "creating a new frame"
            ff = OSDFrame(None)
        else:
            # print "re-using old frame"
            self.frames.remove(ff)
            pass

        self.frames.append(ff)
        ff.ShowOSD(osdText, fontInfo, textColour, outlineColour, alignment, offset, displayNumber, timeout, event, skin)
