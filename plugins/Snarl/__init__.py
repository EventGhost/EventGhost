...
**WARNING:** For proper functioning requires Snarl_ 2.5.1 or 3.1. Snarl 5 and later don't work.
...
class MenuGrid(gridlib.Grid):
    ...
    def __init__(self, parent, plugin, choices = []):
        ...
        self.SetMargins(
            0-wx.SystemSettings.GetMetric(wx.SYS_VSCROLL_X),
        ...
        self.Bind(gridlib.EVT_GRID_CELL_CHANGED, self.OnChange)
...
class SnarlWorkerThread(eg.ThreadWorker):
    ...
    def Setup(self, plugin):
        ...
        try:
            ...
            eg.PrintError("Snarl 2.5.1 or 3.1 not installed?")

        if self.snarl and self.snarl.isRunning():
            res = None
            i = 0
            while res != "SUCCESS":
                res = self.Register()
...
| If you set a Low priority, notification can not also be Sticky!'''
    ...
    def Configure(
        ...
        text = self.text
        panel = eg.ConfigPanel(self)
        panel.indexId = wx.NewIdRef()
        panel.mergeId = wx.NewIdRef()
        panel.durId = wx.NewIdRef()
        panel.percId = wx.NewIdRef()
        panel.replaceId = wx.NewIdRef()
        panel.ids = (
            panel.indexId,
            panel.mergeId,
            panel.durId,
        ...
        def OnRclick(evt):
            ...
            if not hasattr(panel, "popupId0"):
                panel.popupId0 = wx.NewIdRef()
                panel.popupId1 = wx.NewIdRef()
                panel.popupId2 = wx.NewIdRef()
                panel.popupId3 = wx.NewIdRef()
                panel.popups=(
                    panel.popupId0,
                    panel.popupId1,
                    panel.popupId2,
