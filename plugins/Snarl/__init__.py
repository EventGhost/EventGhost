...
class Text:
    ...
    suffLabel = "Suffixes on notification disappearance"
    menuLabel = "User menu"
#===============================================================================
...
class MenuGrid(gridlib.Grid):
    ...
    def SetValue(self, choices):
        ...
        for i in range(len(choices)):
            ...
            self.SetCellValue(i,1,choices[i][1])
#===============================================================================
...
class SuffixGrid(gridlib.Grid):
    ...
    def GetValue(self):
        value = []
        for r in range(self.GetNumberRows()):
            value.append(self.GetCellValue(r, 0))
        return value
    ...
    def Setup(self, plugin):
        ...
        class SubEventHandler(pySnarl.EventHandler):
            ...
            def OnNotificationActionSelected(self, uid, command):
                pld = plugin.uids.index(uid) if uid in plugin.uids else int(uid)
                self.fn("UserMenu.%s" % command, payload = pld)
...
class Snarl(eg.PluginBase):
    ...
    def Configure(self, classes = [], uids = None):
        ...
        suffixSizer.Add(menuCtrl)
        suffixSizer.Add(suffixCtrl)
...
| If you set a Low priority, notification can not also be Sticky!'''
    ...
    class text:
        ...
        labelBox2 = "Event suffixes on disappearance (optional)"
        menu = (
    ...
    def __call__(
        ...
        sffxs = ["Expired","Invoked","Closed"]
        for i in range(3):
            if suffixes[i]:
                sffxs[i] = suffixes[i]
            elif ix and self.plugin.classes[ix][7][i]:
        ...
        return self.plugin.Notify(
            actions if actions else (self.plugin.classes[ix][6] if ix is not None else []),
            "",
            "",
            cls, 
            "",
    ...
    def Configure(
        ...
        rightSizer.Add(rightSizer_1,0,wx.EXPAND)
        rightSizer.Add(rightSizer_2,0,wx.TOP|wx.EXPAND,10)
        mainSizer.Add(sizer,0,wx.EXPAND)
        mainSizer.Add(rightSizer,0,wx.LEFT,10)
        panel.sizer.Add(mainSizer,1)
        ...
        rightSizer_1.Add(menuCtrl)
        rightSizer_2.Add(suffixCtrl)
