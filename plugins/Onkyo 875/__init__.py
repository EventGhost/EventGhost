...
help = """\
Plugin to control the Onkyo TX-SR875 Receiver via RS-232
...
eg.RegisterPlugin(
    ...
    guid='{A61B667C-19C0-48B8-82B5-C7B9944F269F}',
    author="Revised by Fiasco for Onkyo 875 on 07/26/09",
    version="0.1." + "$LastChangedRevision: 809 $".split()[1],
    kind="external",
    description="Control Onkyo TX-SR875 A/V Receiver via RS232",
    help=help,
    canMultiLoad=True,
...
cmdList = (
    ...
    ('VolumeSetEvent', 'Set Master Volume with eg.event.payload[0]', 'MVL', None),
    ('VolumeGet', 'Get Volume', 'MVLQSTN', None),
    ('MuteOn', 'Mute Off', 'AMT00', None),
    ('MuteOff', 'Mute On', 'AMT01', None),
    ('MuteToggle', 'Mute Toggle', 'AMTTG', None),
    ('MuteGet', 'Returns Mute State', 'AMTQSTN', None),
    ...
    ('LMAllChStero', 'All Ch Stereo', 'LMD0C', None),
    ...
    ('LMUSCinema', 'US Cinema', 'LMD50', None),
    ('LMMusicMode', 'US Music Mode', 'LMD51', None),
    ('LMGameMode', 'US Game Mode', 'LMD52', None),
    ('LMPLIIMovie', 'PLII Movie', 'LMD80', None),
    ('LMPLIIMusic', 'PLII Music', 'LMD81', None),
    ('LMN6Cinema', 'Neo:Cinema', 'LMD82', None),
    ('LMN6Music', 'Neo:Music', 'LMD83', None),
    ('LMPLIITHXCinema', 'PLII THX Cinema', 'LMD84', None),
    ('LMN6THXCinema', 'Neo:THX Cinema', 'LMD85', None),
    ('LMPLIIGame', 'PLII Game', 'LMD86', None),
    ('LMNeuralTHX', 'Neural THX', 'LMD88', None),
    ('LMPLIITHXGame', 'PLII THX Game', 'LMD89', None),
    ('LMN6THXGame', 'Neo:THX Game', 'LMD8A', None),
    ...
    ('Z2VolumeSetEvent', 'Set Master Volume with eg.event.payload[0]', 'ZVL', None),
    ...
    ('Z3VolumeSetEvent', 'Set Master Volume with eg.event.payload[0]', 'VL3', None),
...
class CmdAction(eg.ActionClass):
    ...
    def __call__(self):
        ...
        if (self.cmd == '!1MVLUP'):
            repeat = 5
        if (self.cmd == '!1MVLDOWN'):
            repeat = 5
        for i in range(0, repeat):
            event = ''
            if eg.event.payload:
                event = eg.event.payload[0].upper()
            print (self.cmd + event + '-')

            self.plugin.serialThread.Write(self.cmd + str(event) + chr(13))
            time.sleep(.1)
        ...
        while True:
            ...
            if b == "\x1a":
                ...
                if command == "MVL":
                    ...
                    eg.globals.onkyoz1volume = str(int(value, 16))
                    return
                if command == "ZVL":
                    self.TriggerEvent("Zone2Volume", int(value, 16))
                    eg.globals.onkyoz2volume = str(int(value, 16))
                    return
                if command == "VL3":
                    self.TriggerEvent("Zone3Volume", int(value, 16))
                    eg.globals.onkyoz3volume = str(int(value, 16))
                    return
                if command == "TUN":
                    self.TriggerEvent("Frequency", int(value, 16))
                    eg.globals.onkyofrequency = value
                    return
                if command == "PWR":
                    self.TriggerEvent("MasterPower", self.onOffDict[value])
                    eg.globals.onkyoz1power = self.onOffDict[value]
                    return
                if command == "ZPW":
                    self.TriggerEvent("Zone2Power", self.onOffDict[value])
                    eg.globals.onkyoz2power = self.onOffDict[value]
                    return
                if command == "PW3":
                    self.TriggerEvent("Zone3Power", self.onOffDict[value])
                    eg.globals.onkyoz3power = self.onOffDict[value]
                    return
                if command == "AMT":
                    self.TriggerEvent("MasterMute", self.onOffDict[value])
                    eg.globals.onkyoz1mute = self.onOffDict[value]
                    return
                if command == "ZMT":
                    self.TriggerEvent("Zone2Mute", self.onOffDict[value])
                    eg.globals.onkyoz2mute = self.onOffDict[value]
                    return
                if command == "MT3":
                    self.TriggerEvent("Zone3Mute", self.onOffDict[value])
                    eg.globals.onkyoz3mute = self.onOffDict[value]
                    return
                if command == "LMD":
                    self.TriggerEvent("ListeningMode", self.listeningModes[value])
                    eg.globals.onkyolistenmode = self.listeningModes[value]
                    return
                if command == "SLI":
                    print(command)
                    self.TriggerEvent("Zone1Input", self.inputDict[value])
                    eg.globals.onkyoz1input = self.inputDict[value]
                    if value == '25':
                        eg.globals.onkyospectrum = self.inputDict[value]
                    if value == '24':
                        eg.globals.onkyospectrum = self.inputDict[value]
                    return
                if command == "SLZ":
                    print(command)
                    self.TriggerEvent("Zone2Input", self.inputDict[value])
                    eg.globals.onkyoz2input = self.inputDict[value]
                    if value == '25':
                        eg.globals.onkyospectrum = self.inputDict[value]
                    if value == '24':
                        eg.globals.onkyospectrum = self.inputDict[value]
                ...
                if command == "SL3":
                    print(command)
                    self.TriggerEvent("Zone3Input", self.inputDict[value])
                    eg.globals.onkyoz3input = self.inputDict[value]
                    if value == '25':
                        eg.globals.onkyospectrum = self.inputDict[value]
                    if value == '24':
                        eg.globals.onkyospectrum = self.inputDict[value]
                ...
                if command == "SLP":
                    ...
                    eg.globals.onkyosleeptime = str(payload)
                    return
