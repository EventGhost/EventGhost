...
def GetHwnd():
    try:
        vh = eg.WindowMatcher(
            program=None,
            winName=u'Virtual house',
            winClass=u'wxWindowNR',
    ...
    while vh:
        ...
        if GetWindowText(panel) == "FWS panel EG":
            return panel
...
class RoomPanel(wx.Panel):
    def __init__(self, parent, colour, label, plugin):
        ...
        self.bitmap.SetToolTip(text.lampHint)
...
class VirtualHouseFrame(wx.Frame):
    ...
    def __init__(
        ...
        bmp_off = wx.Bitmap(wx.Image(stream_off))
        bmp_on = wx.Bitmap(wx.Image(stream_on))
        bmp_empty = wx.Bitmap(wx.Image(stream_empty))
