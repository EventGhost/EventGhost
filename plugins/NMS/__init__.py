...
# -------------------------------------------
# 0.1.2 by Pako 2015-04-15 16:31 GMT+1
#     - bugfix - (text -> self.text)
...
import wx
from PIL.Image import ANTIALIAS, open as openImage
...
class ApiListCtrl(wx.ListCtrl):
    ...
    def __init__(self, parent, id, plugin, size):
        ...
        idx0 = il.Add(wx.Bitmap(wx.Image(stream_sp)))
        stream_dr = StringIO(b64decode(DROID))
        idx1 = il.Add(wx.Bitmap(wx.Image(stream_dr)))
        stream_fr = StringIO(b64decode(FRUIT))
        idx2 = il.Add(wx.Bitmap(wx.Image(stream_fr)))
        self.il = il
    ...
    def Set(self):
        ...
        for i, itm in enumerate(lst):
            ...
            self.InsertItem(i, " ", ix)
            self.SetItem(i, 1, itm[0])
...
class NMS(eg.PluginClass):
    ...
    def Configure(self, apikeys=[], devel="", provid=""):
        ...
        dialogButton = wx.Button(panel, -1, text.dialog + " ...")
    ...
    def UpdateIcon(self, xmlNode, ix):
        ...
        def Traverse(item):
            if tc.GetItemData(item) == xmlNode:
                return item
    ...
    def GetApiKeyLsts(self, apikeys, apis):
        ...
        for api in apikeys:
            ...
            if api in keys:
                ...
                eg.PrintError(self.text.errMess1 % api)
...
class Notify(eg.ActionBase):
 ...
 the API keys are bad (and success if at least one API is OK)"""
    ...
    def Configure(
        ...
        batchModeLabel.SetToolTip(text.batchTip)
        batchModeCtrl = wx.CheckBox(panel, -1, "")
        batchModeCtrl.SetValue(batch_mode)
        batchModeCtrl.SetToolTip(text.batchTip)
