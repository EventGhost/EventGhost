...
eg.RegisterPlugin(
    name="CyberPower UPS HID",
    guid='{9E4D4219-A7EA-4514-9C92-618DBAA2DE19}',
    author="Scope",
    version="0.1",
    kind="remote",
    canMultiLoad=True,
    description=(
        'Extremely rough plugin based on Generic HID device plugin to handle CyberPower UPS Model BS850E.'
        'It does only support two events right now. Which is whether the UPS '
        'is running on battery or AC.'
    ),
...
class CyberPowerUPS(eg.PluginClass):
    currentPowerSupply = 5
    ...
    def ButtonCallback(self, data):
        ...
        if len(data) >= 1:
            ...
            if data[0] == 7:
                if self.currentPowerSupply != 7:
                    self.TriggerEvent("Power.Battery")
                    self.currentPowerSupply = 7
            if data[0] == 5:
                if self.currentPowerSupply != 5:
                    self.TriggerEvent("Power.AC")
                    self.currentPowerSupply = 5
    ...
    def Configure(self,
        ...
        for item in deviceList:
            # if item.vendorString == "CPS" and item.productString == "BS850E":
            if item.vendorString == "CPS":
                print 'Found device.'
