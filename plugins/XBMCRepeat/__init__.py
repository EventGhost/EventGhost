...
WINDOWS = (
    (eg.ActionGroup, "Windows", "Windows", None, (
        ...
        ("Musicplaylisteditor", "Musicplaylisteditor", "WINDOW_MUSIC_PLAYLIST_EDITOR",
        ...
        ("Visualisationpresetlist", "Visualisationpresetlist", "WINDOW_DIALOG_VIS_PRESET_LIST",
        ...
        ("Smartplaylisteditor", "Smartplaylisteditor", "WINDOW_DIALOG_SMART_PLAYLIST_EDITOR",
         "Activatewindow(Smartplaylisteditor)"),
        ("Smartplaylistrule", "Smartplaylistrule", "WINDOW_DIALOG_SMART_PLAYLIST_RULE",
...
GENERAL_ACTIONS = (
    (eg.ActionGroup, "General", "General", None, (
        ...
        ("Info", "Info", "Show the information about the currently highlighted item, or currently playing item.",
         "info"),
...
MEDIA_PLAYING_ACTIONS = (
    (eg.ActionGroup, "MediaPlaying", "Media playing", None, (
        ...
        ("FastForward", "FastForward", "Toggle the fastforward speed between normal play, 2x, 4x, 8x, 16x, and 32x.",
         "fastforward"),
        ...
        ("CodecInfo", "CodecInfo",
         ...
         "codecinfo"),
        ("AnalogSeekForward", "AnalogSeekForward", "Variable speed seeking for analog keys (stick or triggers)",
         "analogseekforward"),
        ("AnalogSeekBack", "AnalogSeekBack", "Variable speed seeking for analog keys (stick or triggers)",
         "analogseekback"),
        ("AnalogFastForward", "AnalogFastForward", "Variable speed fast forward for analog keys (stick or triggers)",
         "analogfastforward"),
        ...
        ("LastFMLove", "LastFM.Love", "Add the current playing last.fm radio track to the last.fm loved tracks",
         "LastFM.Love"),
...
FULLSCREEN_VIDEO_ACTIONS = (
    (eg.ActionGroup, "FullscreenVideo", "FullScreen Video", None, (
        ...
        (
        "AspectRatio", "AspectRatio", "Toggle through the various aspect ratio modes (Normal is the preferred option).",
        "aspectratio"),
        ("ShowSubtitles", "ShowSubtitles", "Toggles whether subtitles are shown or not.", "showsubtitles"),
        ("NextSubtitle", "NextSubtitle", "Change to the next subtitle language, if there is more than one.",
         "nextsubtitle"),
        ('Subtitledelay', 'SubtitleDelay', 'Show subtitle delay slider', 'subtitledelay'),
        ("SubtitleDelayMinus", "SubtitleDelayMinus",
         "Decrease the delay amount of subtitles (use if subtitles are displaying too late)", "subtitledelayminus"),
        ("SubtitleDelayPlus", "SubtitleDelayPlus",
         "Increase the delay amount of subtitles (use if subtitles are displaying too early)", "subtitledelayplus"),
        ('Audiodelay', 'AudioDelay', 'Show audio delay slider', 'audiodelay'),
        ("AudioDelayMinus", "AudioDelayMinus",
         "Decrease the delay amount of audio (use if audio is being heard too early)", "audiodelayminus"),
        (
        "AudioDelayPlus", "AudioDelayPlus", "Increase the delay amount of audio (use if audio is being heard too late)",
        "audiodelayplus"),
        ("AudioNextLanguage", "AudioNextLanguage",
         "Change to the next audio track in a video with multiple audio tracks.", "audionextlanguage"),
        ...
        ('Increasepar', 'IncreasePAR', 'Used in video fullscreen to increase the pixel aspect ratio (stretch).',
         'increasepar'),
        ('Decreasepar', 'DecreasePAR', 'Used in video fullscreen to decrease the pixel aspect ratio (stretch).',
         'decreasepar'),
    )),
)
...
SLIDESHOW_ACTIONS = (
    (eg.ActionGroup, "PictureSlideshow", "Picture slideshow", None, (
        ...
        ("ZoomOut", "ZoomOut", "Used in picture, slideshow or video fullscreen to zoom out of the current image/video.",
         "zoomout"),
        ("ZoomIn", "ZoomIn", "Used in picture, slideshow or video fullscreen to zoom in to the current image/video.",
         "zoomin"),
...
ON_SCREEN_KEYBOARD_ACTIONS = (
    (eg.ActionGroup, "On-screenKeyboard", "On-screen keyboard", None, (
        ...
        ("CursorLeft", "CursorLeft", "Used in Virtual Keyboard to move the current cursor point to the left",
         "cursorleft"),
        ("CursorRight", "CursorRight", "Used in Virtual Keyboard to move the current cursor point to the right",
         "cursorright"),
    )),
)
...
SHUTDOWN_ACTIONS = (
    (eg.ActionGroup, "ShutdownRelated", "Shutdown related", None, (
        ...
        ("Shutdown", "Shutdown Computer",
         "Trigger default Shutdown action defined in System Settings, Default Quit on Windows.", "shutdown"),
...
UNCATEGORIZED_ACTIONS = (
    (eg.ActionGroup, "UncategorizedActions", "Uncategorized actions", None, (
        ("JumpSMS2", "JumpSMS2", "Jump through a list using SMS-style input (eg press 2 twice to jump to the B's.)",
         "jumpsms2"),
        ("JumpSMS3", "JumpSMS3", "Jump through a list using SMS-style input (eg press 2 twice to jump to the B's.)",
         "jumpsms3"),
        ("JumpSMS4", "JumpSMS4", "Jump through a list using SMS-style input (eg press 2 twice to jump to the B's.)",
         "jumpsms4"),
        ("JumpSMS5", "JumpSMS5", "Jump through a list using SMS-style input (eg press 2 twice to jump to the B's.)",
         "jumpsms5"),
        ("JumpSMS6", "JumpSMS6", "Jump through a list using SMS-style input (eg press 2 twice to jump to the B's.)",
         "jumpsms6"),
        ("JumpSMS7", "JumpSMS7", "Jump through a list using SMS-style input (eg press 2 twice to jump to the B's.)",
         "jumpsms7"),
        ("JumpSMS8", "JumpSMS8", "Jump through a list using SMS-style input (eg press 2 twice to jump to the B's.)",
         "jumpsms8"),
        ("JumpSMS9", "JumpSMS9", "Jump through a list using SMS-style input (eg press 2 twice to jump to the B's.)",
         "jumpsms9"),
        ...
        ('Container.Update', 'Container.Update',
         'Update current listing. Send Container.Update(path,replace) to reset the path history', 'Container.Update'),
        ('Container.NextViewMode', 'Container.NextViewMode', 'Move to the next view type (and refresh the listing)',
         'Container.NextViewMode'),
        ('Container.PreviousViewMode', 'Container.PreviousViewMode',
         'Move to the previous view type (and refresh the listing)', 'Container.PreviousViewMode'),
        ('Container.NextSortMethod', 'Container.NextSortMethod', 'Change to the next sort method',
         'Container.NextSortMethod'),
        ('Container.PreviousSortMethod', 'Container.PreviousSortMethod', 'Change to the previous sort method',
        ...
        ('Weather.LocationPrevious', 'Weather.LocationPrevious', 'Switch to previous weather location',
    ...
    def Configure(self, Function="Help", Parameters=""):
        ...
        def UpdateFunctions():
            def GetFunctions():
                try:
                    ...
                    Builtinslist = Builtinslines[
                                   Builtinslines.index("const BUILT_IN commands[] = {") + 1:Builtinslines.index("};")]

                    FunctionList = []
                    for Function in Builtinslist:
                        try:
                            FunctionList.append((Function.strip(' {},').split(',')[0].strip('" '),
                    ...
                    for table in xml.dom.minidom.parseString(Page2).getElementsByTagName("table")[1:2]:
                        for tr in table.getElementsByTagName("tr"):
                            for code in tr.getElementsByTagName("code")[0:1]:
                                ActionDict[XMLText(code).strip().split('(')[0]] = (
                                XMLText(code).strip(), XMLText(code.parentNode.nextSibling.nextSibling).strip())
            ...
            try:
                ...
                for Function, Parameters, Description in sorted(FunctionsList):
                    try:
                        ...
                        Syntax = Function
                    BuiltInFunctionList[Function] = {'Syntax': Syntax, 'Description': Description,
        ...
        def OnFunctionChange(event):
            if event.GetEventObject() == panel.combo_box_function:
                ...
                panel.label_description.SetLabel(
                    BuiltInFunctionList[panel.combo_box_function.GetValue()]['Description'])

        def initPanel(self):
            self.combo_box_function = wx.ComboBox(self, wx.ID_ANY, value=Function,
                                                  choices=sorted(BuiltInFunctionList.keys()), style=wx.CB_READONLY)
            ...
            self.text_ctrl_syntax = wx.TextCtrl(self, wx.ID_ANY, BuiltInFunctionList[Function]['Syntax'],
                                                style=wx.TE_READONLY | wx.BORDER_NONE)
            self.sizer_syntax_staticbox = wx.StaticBox(self, wx.ID_ANY, "Syntax")
            self.label_description = wx.TextCtrl(self, wx.ID_ANY, BuiltInFunctionList[Function]['Description'],
        ...
        def setPanelProperties(self):
            ...
            self.button_update.SetToolTip("Update \"BuiltInFunctions\" form Kodis website.")
...
#				print 'Failed to open: httpapi.dat'
        ...
        def UpdateCommands():
            ...
            try:
                doc = xml.dom.minidom.parseString(
                    urllib2.urlopen(urllib2.Request('http://kodi.wiki/view/Web_Server_HTTP_API')).read())
            except (urllib2.HTTPError, urllib2.URLError):
                print "Connect error"
            else:
        ...
        panel = eg.ConfigPanel()
        print "--------------"
        print httpapi.Headers
        print "--------------"
        print category
        try:
            choice = httpapi.Headers[category]
        except IndexError:
            choice = 'No categorys'
        HBoxControl = wx.ComboBox(
            panel, -1, value=choice, choices=httpapi.Headers, style=wx.CB_READONLY
        )
        try:
            choices = httpapi.Commands[category][0]
        except IndexError:
            choices = ['No commands']
        comboBoxControl = wx.ComboBox(panel, -1, value=command, choices=choices)
        ...
        if (comboBoxControl.GetSelection() != -1):
            try:
                selection = httpapi.Commands[category][1][comboBoxControl.GetSelection()]
            except IndexError:
                selection = 0
            syntax = wx.TextCtrl(
                panel, -1, selection, (1, 70), size=(500, -1), style=wx.TE_READONLY
            )
        else:
        ...
        if (comboBoxControl.GetSelection() != -1):
            try:
                selection = httpapi.Commands[category][2][comboBoxControl.GetSelection()]
            except IndexError:
                selection = 0
            description = wx.StaticText(
                panel, -1, selection, (5, 105), style=wx.ALIGN_LEFT
            )
        else:
...
class JSONRPC(eg.ActionClass):
    ...
    def Configure(self, method="JSONRPC.Introspect", param="", log=True, wait=True):
        ...
        def UpdateMethods():
            ...
            if responce:
                ...
                if responce['result']['version'] > 2:
                            ...
                            for method in responce['result']['methods']:
                                ...
                                if responce['result']['methods'][method].has_key('description'):
                                    jsonrpc.Descriptions[namespace].append(
                                        responce['result']['methods'][method]['description'])
                                else:
                                    jsonrpc.Descriptions[namespace].append('')
                            try:
                    ...
                    responce = self.plugin.JSON_RPC.send('JSONRPC.Introspect', json.loads(
        ...
        def OnMethodChange(event):
            if event.GetEventObject() == comboBoxControl:
                description.SetLabel(
                    jsonrpc.Descriptions[jsonrpc.Namespaces[HBoxControl.GetSelection()]][event.GetSelection()])
                description.Wrap(480)
            else:
        ...
        try:
            ...
            UpdateMethods()
        HBoxControl = wx.ComboBox(panel, -1, value=method[:method.find('.')], choices=jsonrpc.Namespaces,
                                  style=wx.CB_READONLY)
        comboBoxControl = wx.ComboBox(panel, -1, value=method[method.find('.') + 1:],
                                      choices=jsonrpc.Methods[jsonrpc.Namespaces[HBoxControl.GetSelection()]],
                                      style=wx.CB_READONLY)
        ...
        if (comboBoxControl.GetSelection() != -1):
            description = wx.StaticText(panel, -1, jsonrpc.Descriptions[jsonrpc.Namespaces[HBoxControl.GetSelection()]][
        ...
        while panel.Affirmed():
            ...
            panel.SetResult(HBoxControl.GetValue() + '.' + comboBoxControl.GetValue(), textControl2.GetValue(),
                            CheckBox.GetValue(), CheckBox2.GetValue())
    ...
    for result in ssdpResultList:
        ...
        for modelName in doc.getElementsByTagName("modelName"):
            if modelName.firstChild.data in ('XBMC Media Center', 'Kodi'):
                XBMCResultList[urlparse(doc.getElementsByTagName("presentationURL")[0].firstChild.data).netloc] = \
...
class XBMC2(eg.PluginClass):
    ...
    def __init__(self):
        ...
        try:
            MANUALLYUPDATED_ACTIONS = [
                [eg.ActionGroup, "ManuallyUpdated", "Manually Updated", None, readData('actions.dat')]]
        except IOError:
            # eg.PrintError('Failed to open: httpapi.dat')
            pass
        else:
    ...
    def Configure(self, pluginConfig={}, *args):
        def UpdateActions(event):
                    ...
                    for code in tr.getElementsByTagName("code")[0:1]:
                        ...
                        if '2-9' in XMLText(code).strip():
                            for i in range(2, 10):
                                ...
                                ActionDict[(XMLText(code).strip()[:-5] + str(i)).lower()] = (
                                (XMLText(code).strip()[:-5] + str(i)),
                                XMLText(code.parentNode.nextSibling.nextSibling).strip())
            ...
            for a in GENERAL_ACTIONS[0][4] + MEDIA_PLAYING_ACTIONS[0][4] + PLAYLIST_ACTIONS[0][4] + \
                     FULLSCREEN_VIDEO_ACTIONS[0][4] + SLIDESHOW_ACTIONS[0][4] + CALIBRATION_ACTIONS[0][4] + \
                     FILEMANAGER_ACTIONS[0][4] + ON_SCREEN_KEYBOARD_ACTIONS[0][4] + VISUALISATION_ACTIONS[0][4] + \
        ...
        def ConnectionTest(event):
            ...
            self.JSON_RPC.connect(ip=panel.combo_box_IP.GetValue().split(':')[0],
            ...
            try:
                result = self.JSON_RPC.send('GUI.ShowNotification', ast.literal_eval(
            ...
            #	eg.PrintError('XBMC2: JSON-RPC error: ', str(result['error']['message']))
                ...
                self.HTTP_API.connect(ip=panel.combo_box_IP.GetValue().split(':')[0],
                                      port=panel.combo_box_IP.GetValue().split(':')[1],
                                      username=panel.text_ctrl_Username.GetValue(),
                                      password=panel.text_ctrl_Password.GetValue())
                try:
        ...
        def initPanel(self):
            self.combo_box_IP = wx.ComboBox(self, wx.ID_ANY,
                                            value=pluginConfig['XBMC']['ip'] + ':' + str(pluginConfig['XBMC']['port']),
                                            choices=["127.0.0.1:80"], style=wx.CB_DROPDOWN)
            ...
            self.text_ctrl_Password = wx.TextCtrl(self, wx.ID_ANY, pluginConfig['XBMC']['password'],
            ...
            self.spin_ctrl_EventServerPort = wx.SpinCtrl(self, wx.ID_ANY, str(pluginConfig['EventServer']['port']),
                                                         min=0, max=65535)
            ...
            self.spin_ctrl_JSONRPCPort = wx.SpinCtrl(self, wx.ID_ANY, str(pluginConfig['JSONRPC']['port']), min=0,
                                                     max=65535)
            ...
            self.spin_ctrl_BroadcastPort = wx.SpinCtrl(self, wx.ID_ANY, str(pluginConfig['Broadcast']['port']), min=0,
                                                       max=65535)
        ...
        def setPanelProperties(self):
            ...
            self.combo_box_IP.SetToolTip("IP address of the XBMC you want to control.")
            self.button_IPTest.SetToolTip("Test to connect to XBMC")

            self.button_UpdateActions.SetToolTip(
                'Add any new XBMC actions to a category "Manually Updated". You need to restart EventGhost for the new actions to be visible')

            self.button_Search.SetToolTip("Search for any XBMCs that are running and reachable over the LAN.")
            self.text_ctrl_Username.SetToolTip("Username that are specified in XBMC")
            self.text_ctrl_Password.SetToolTip("Password that are specified in XBMC")
            ...
            self.spin_ctrl_EventServerPort.SetToolTip("Port used by XBMC to recieve notifications")
            self.checkbox_JSONRPCEnable.SetToolTip("Enable JSON-RPC notifications")
            ...
            self.spin_ctrl_JSONRPCPort.SetToolTip("Port used by XBMC to recieve notifications")
            self.spin_ctrl_Retrys.SetMinSize((50, -1))
            self.spin_ctrl_Time.SetMinSize((50, -1))
            self.checkbox_logRawEvents.SetToolTip("Show any events from XBMC in the log, exactly as XBMC sends them.")

            self.checkbox_logDebug.SetToolTip(
    ...
    def __start__(self, pluginConfig={}, *args):
        ...
        self.JSON_RPC.connect(ip=pluginConfig['XBMC']['ip'], port=pluginConfig['XBMC']['port'],
                              username=pluginConfig['XBMC']['username'], password=pluginConfig['XBMC']['password'])
        ...
        if self.pluginConfig['JSONRPC']['enable']:
            try:
                ...
                self.JSONRPCNotificationsThread = Thread(target=self.JSONRPCNotifications,
    ...
    def JSONRPCNotifications(self, stopJSONRPCNotifications):
        ...
        def WaitForXBMC(Socket):
            ...
            while not (stopJSONRPCNotifications.isSet() or XBMCDetected):
                ...
                # headers = Headers(sock.recv(4096))
                    ...
                    try:
                        if debug:
                            with open(os.path.join(eg.folderPath.RoamingAppData, 'EventGhost', 'plugins', 'XBMC2',
                                                   'ssdp.log'), 'a+') as f:
                                    ...
                                    try:
                                        ...
                                        for modelName in doc.getElementsByTagName("modelName"):
                                            if modelName.firstChild.data in ('Kodi', 'XBMC Media Center', 'XBMC'):
                                                if debug:
                                                    with open(os.path.join(eg.folderPath.RoamingAppData, 'EventGhost',
                                                                           'plugins', 'XBMC2', 'ssdp.log'), 'a+') as f:
                                                        f.write(data)
                                                ...
                                                if self.pluginConfig['XBMC']['ip'] == '127.0.0.1':
                                                    for ip in interface_addresses():
                                                        ...
                                                        if urlparse(doc.getElementsByTagName("presentationURL")[
                                                                        0].firstChild.data).netloc.split(":")[0] == ip:
                                                            try:
                                                                if urlparse(doc.getElementsByTagName("presentationURL")[
                                                                                0].firstChild.data).netloc.split(":")[
                                                                    1] == str(self.pluginConfig['XBMC']['port']):
                                                                    XBMCDetected = True
                                                                    break
                                                                else:
                                                                    continue
                                                    ...
                                                    if debug:
                                                        print 'XBMC2: SSDP address:', urlparse(
                                                            doc.getElementsByTagName("presentationURL")[
                                                                0].firstChild.data).netloc
                                                    if urlparse(doc.getElementsByTagName("presentationURL")[
                                                                    0].firstChild.data).netloc == \
                                                        self.pluginConfig['XBMC']['ip'] + ':' + str(
                                                        self.pluginConfig['XBMC']['port']):
                                                        XBMCDetected = True
                                            else:
                                                if debug:
                                                    with open(os.path.join(eg.folderPath.RoamingAppData, 'EventGhost',
        ...
        while not stopJSONRPCNotifications.isSet():
            ...
            try:
                ...
                print "XBMC2: Connected to XBMC (", self.pluginConfig['XBMC']['ip'], ":", self.pluginConfig['JSONRPC'][
                    'port'], "), ready to recive JSON-RPC notifications."
                                ...
                                try:
                                    ...
                                    try:
                                        if 'notifications' in message['result']:
                                            if debug:
                                                print 'XBMC2: JSON-RPC responce: "notifications":', message['result'][
                                                    'notifications']
                                            continue
                                    except KeyError:
                                        if debug:
                                            eg.PrintError(
                                                'XBMC2: Error: JSON-RPC responce, "result" missing ' + repr(message))
                                    else:
                                        if message['result'] == 'pong':
                                            ...
                                            eg.PrintError(
                                                'XBMC2: Error: JSON-RPC responce, "pong" missing ' + repr(message))
                                            self.PrintError(
