...
class Scene(eg.ActionBase):
    ...
    def __call__(self, scene):
        ...
        scene = self.plugin.VDL.GetID(scene=scene)
        if scene: self.plugin.SERVER.send(scene=scene)

    def Configure(self, scene=" "):

        text = self.text.Scene
        panel = eg.ConfigPanel()
        sceneCtrl = SceneConfigPanel()(self, panel)

        sceneCtrl.AddItems(text.ScenesBox, '-5', scene=scene)
        panel.sizer.Add(sceneCtrl, 0, wx.EXPAND)

        while panel.Affirmed():
            panel.SetResult(sceneCtrl.GetStringSelection(),)
