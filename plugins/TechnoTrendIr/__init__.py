...
#
#
# $LastChangedDate: 2007-04-24 00:22:30 +0200 (Di, 24 Apr 2007) $
...
class TTIR(eg.PluginClass):
    ...
    def IrCallback(self, context, buf, length, irMode, hOpen, devIdx):
        print repr(context), repr(buf), repr(length), repr(irMode)
        if irMode == 3:
            
            code = cast(buf, POINTER(DWORD))
            irMode2 = self.dll.ir_Get_AllCode_IRMode(code[0]) & 0xffffffff
            print irMode2, self.dll.irGetIRModeDescription(irMode2)
        pass
